...
def plot_image_grid(images_np,
    ...
    assert (n_channels == 3) \
        or (n_channels == 1), "images should have 1 or 3 channels"

    images_np = [x if (x.shape[0] == n_channels)
                 else np.concatenate([x, x, x], axis=0) for x in images_np]
    ...
    # plot.savefig() should be called before plt.show()
    # since after calling plt.show() if we call plt.savefig()
    # we get an white image this is coz a new blank canvas is
    # created
    # https://stackoverflow.com/questions/9012487/matplotlib-pyplot-savefig-outputs-blank-image

    fig = plt.gcf()

    if save_plot and fname is not None:
        fig.savefig(fname,
                    transparent=True,
                    bbox_inches='tight',
                    pad_inches=0)
...
def get_noise(input_depth, method, spatial_size, noise_type='u', var=1. / 10):
    """
    Returns a pytorch.Variable of
    size (1 x `input_depth` x `spatial_size[0]` x `spatial_size[1]`)
